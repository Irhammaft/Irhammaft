 ...
 */
#define pr_fmt(fmt) "flashv2: %s: " fmt, __func__ include <linux/module.h> include 
#<linux/init.h> include <linux/kernel.h> include <linux/errno.h> include 
#<linux/delay.h> include <linux/slab.h> include <linux/of.h> include 
#<linux/of_irq.h> include <linux/of_gpio.h> include <linux/gpio.h> include 
#<linux/regmap.h> include <linux/power_supply.h> include <linux/platform_device.h> 
#include <linux/interrupt.h> include <linux/regulator/consumer.h> include 
#<linux/leds-qpnp-flash.h> include <linux/leds-qpnp-flash-v2.h> include 
#<linux/qpnp/qpnp-revid.h> include <linux/log2.h> include "leds.h" define 
#FLASH_LED_REG_LED_STATUS1(base) (base + 0x08) define 
#FLASH_LED_REG_LED_STATUS2(base) (base + 0x09) define 
#FLASH_LED_VPH_DROOP_FAULT_MASK BIT(4) define FLASH_LED_REG_INT_RT_STS(base) (base 
#+ 0x10) define FLASH_LED_REG_SAFETY_TMR(base) (base + 0x40) define 
#FLASH_LED_SAFETY_TMR_ENABLE BIT(7) define FLASH_LED_REG_TGR_CURRENT(base) (base + 
#0x43) define FLASH_LED_REG_MOD_CTRL(base) (base + 0x46) define 
#FLASH_LED_MOD_CTRL_MASK BIT(7) define FLASH_LED_MOD_ENABLE BIT(7) define 
#FLASH_LED_REG_IRES(base) (base + 0x47) define FLASH_LED_REG_STROBE_CFG(base) 
#(base + 0x48) define FLASH_LED_STROBE_MASK GENMASK(1, 0) define 
#FLASH_LED_REG_STROBE_CTRL(base) (base + 0x49) define 
#FLASH_LED_HW_SW_STROBE_SEL_BIT BIT(2) define FLASH_HW_STROBE_MASK GENMASK(2, 0) 
#define FLASH_LED_EN_LED_CTRL(base) (base + 0x4C) define FLASH_LED_ENABLE BIT(0) 
#define FLASH_LED_REG_HDRM_PRGM(base) (base + 0x4D) define FLASH_LED_HDRM_VOL_MASK 
#GENMASK(7, 4) define FLASH_LED_HDRM_VOL_SHIFT 4 define 
#FLASH_LED_REG_HDRM_AUTO_MODE_CTRL(base) (base + 0x50) define 
#FLASH_LED_REG_WARMUP_DELAY(base) (base + 0x51) define 
#FLASH_LED_REG_ISC_DELAY(base) (base + 0x52) define 
#FLASH_LED_ISC_WARMUP_DELAY_MASK GENMASK(1, 0) define 
#FLASH_LED_ISC_WARMUP_DELAY_SHIFT 6 define FLASH_LED_REG_THERMAL_RMP_DN_RATE(base) 
#(base + 0x55) define THERMAL_OTST1_RAMP_CTRL_MASK BIT(7) define 
#THERMAL_OTST1_RAMP_CTRL_SHIFT 7 define THERMAL_DERATE_SLOW_SHIFT 4 define 
#THERMAL_DERATE_SLOW_MASK GENMASK(6, 4) define THERMAL_DERATE_FAST_MASK GENMASK(2, 
#0) define FLASH_LED_REG_THERMAL_THRSH1(base) (base + 0x56) define 
#FLASH_LED_THERMAL_THRSH_MASK GENMASK(2, 0) define 
#FLASH_LED_REG_THERMAL_THRSH2(base) (base + 0x57) define 
#FLASH_LED_REG_THERMAL_THRSH3(base) (base + 0x58) define 
#FLASH_LED_REG_THERMAL_HYSTERESIS(base) (base + 0x59) define 
#FLASH_LED_THERMAL_HYSTERESIS_MASK GENMASK(1, 0) define 
#FLASH_LED_REG_THERMAL_DEBOUNCE(base) (base + 0x5A) define 
#FLASH_LED_THERMAL_DEBOUNCE_MASK GENMASK(1, 0) define 
#FLASH_LED_REG_VPH_DROOP_THRESHOLD(base) (base + 0x61) define 
#FLASH_LED_VPH_DROOP_HYSTERESIS_MASK GENMASK(5, 4) define 
#FLASH_LED_VPH_DROOP_THRESHOLD_MASK GENMASK(2, 0) define 
#FLASH_LED_VPH_DROOP_HYST_SHIFT 4 define FLASH_LED_REG_VPH_DROOP_DEBOUNCE(base) 
#(base + 0x62) define FLASH_LED_VPH_DROOP_DEBOUNCE_MASK GENMASK(1, 0) define 
#FLASH_LED_REG_ILED_GRT_THRSH(base) (base + 0x67) define 
#FLASH_LED_ILED_GRT_THRSH_MASK GENMASK(5, 0) define 
#FLASH_LED_REG_LED1N2_ICLAMP_LOW(base) (base + 0x68) define 
#FLASH_LED_REG_LED1N2_ICLAMP_MID(base) (base + 0x69) define 
#FLASH_LED_REG_LED3_ICLAMP_LOW(base) (base + 0x6A) define 
#FLASH_LED_REG_LED3_ICLAMP_MID(base) (base + 0x6B) define FLASH_LED_CURRENT_MASK 
#GENMASK(6, 0) define FLASH_LED_REG_MITIGATION_SEL(base) (base + 0x6E) define 
#FLASH_LED_CHGR_MITIGATION_SEL_MASK GENMASK(5, 4) define 
#FLASH_LED_LMH_MITIGATION_SEL_MASK GENMASK(1, 0) define 
#FLASH_LED_REG_MITIGATION_SW(base) (base + 0x6F) define 
#FLASH_LED_LMH_MITIGATION_EN_MASK BIT(0) define FLASH_LED_CHGR_MITIGATION_EN_MASK 
#BIT(4) define FLASH_LED_CHGR_MITIGATION_ENABLE BIT(4) define 
#FLASH_LED_REG_LMH_LEVEL(base) (base + 0x70) define FLASH_LED_LMH_LEVEL_MASK 
#GENMASK(1, 0) define FLASH_LED_REG_MULTI_STROBE_CTRL(base) (base + 0x71) define 
#LED3_FLASH_ONCE_ONLY_BIT BIT(1) define LED1N2_FLASH_ONCE_ONLY_BIT BIT(0) define 
#FLASH_LED_REG_LPG_INPUT_CTRL(base) (base + 0x72) define LPG_INPUT_SEL_BIT BIT(0) 
#define FLASH_LED_REG_CURRENT_DERATE_EN(base) (base + 0x76) define 
#FLASH_LED_CURRENT_DERATE_EN_MASK GENMASK(2, 0) define 
#VPH_DROOP_DEBOUNCE_US_TO_VAL(val_us) (val_us / 8) define 
#VPH_DROOP_HYST_MV_TO_VAL(val_mv) (val_mv / 25) define 
#VPH_DROOP_THRESH_VAL_TO_UV(val) ((val + 25) * 100000) define 
#MITIGATION_THRSH_MA_TO_VAL(val_ma) (val_ma / 100) define 
#THERMAL_HYST_TEMP_TO_VAL(val, divisor) (val / divisor) define 
#FLASH_LED_WARMUP_DELAY_DEFAULT 2 define FLASH_LED_ISC_DELAY_DEFAULT 3 define 
#FLASH_LED_VPH_DROOP_DEBOUNCE_DEFAULT 2 define FLASH_LED_VPH_DROOP_HYST_DEFAULT 2 
#define FLASH_LED_VPH_DROOP_THRESH_DEFAULT 5 define 
#BHARGER_FLASH_LED_VPH_DROOP_THRESH_DEFAULT 7 define FLASH_LED_DEBOUNCE_MAX 3 
#define FLASH_LED_HYSTERESIS_MAX 3 define FLASH_LED_VPH_DROOP_THRESH_MAX 7 define 
#THERMAL_DERATE_SLOW_MAX 314592 define THERMAL_DERATE_FAST_MAX 512 define 
#THERMAL_DEBOUNCE_TIME_MAX 64 define THERMAL_DERATE_HYSTERESIS_MAX 3 define 
#FLASH_LED_THERMAL_THRSH_MIN 3 define FLASH_LED_THERMAL_THRSH_MAX 7 define 
#FLASH_LED_THERMAL_OTST_LEVELS 3 define FLASH_LED_VLED_MAX_DEFAULT_UV 3500000 
#define FLASH_LED_IBATT_OCP_THRESH_DEFAULT_UA 4500000 define 
#FLASH_LED_RPARA_DEFAULT_UOHM 0 define FLASH_LED_LMH_LEVEL_DEFAULT 0 define 
#FLASH_LED_LMH_MITIGATION_ENABLE 1 define FLASH_LED_LMH_MITIGATION_DISABLE 0 
#define FLASH_LED_CHGR_MITIGATION_DISABLE 0 define 
#FLASH_LED_LMH_MITIGATION_SEL_DEFAULT 2 define FLASH_LED_MITIGATION_SEL_MAX 2 
#define FLASH_LED_CHGR_MITIGATION_SEL_SHIFT 4 define 
#FLASH_LED_CHGR_MITIGATION_THRSH_DEFAULT 0xA define 
#FLASH_LED_CHGR_MITIGATION_THRSH_MAX 0x1F define 
#FLASH_LED_LMH_OCV_THRESH_DEFAULT_UV 3700000 define 
#FLASH_LED_LMH_RBATT_THRESH_DEFAULT_UOHM 400000 define FLASH_LED_IRES_BASE 3 
#define FLASH_LED_IRES_DIVISOR 2500 define FLASH_LED_IRES_MIN_UA 5000 define 
#FLASH_LED_IRES_DEFAULT_UA 12500 define FLASH_LED_IRES_DEFAULT_VAL 0x00 define 
#FLASH_LED_HDRM_VOL_DEFAULT_MV 0x80 define FLASH_LED_HDRM_VOL_HI_LO_WIN_DEFAULT_MV 
#0x04 define FLASH_LED_HDRM_VOL_BASE_MV 125 define FLASH_LED_HDRM_VOL_STEP_MV 25 
#define FLASH_LED_STROBE_CFG_DEFAULT 0x00 define FLASH_LED_HW_STROBE_OPTION_1 0x00 
#define FLASH_LED_HW_STROBE_OPTION_2 0x01 define FLASH_LED_HW_STROBE_OPTION_3 0x02 
#define FLASH_LED_DISABLE 0x00 define FLASH_LED_SAFETY_TMR_DISABLED 0x13 define 
#FLASH_LED_MAX_TOTAL_CURRENT_MA 3750 define FLASH_LED_IRES5P0_MAX_CURR_MA 640 
#define FLASH_LED_IRES7P5_MAX_CURR_MA 960 define FLASH_LED_IRES10P0_MAX_CURR_MA 
#1280 define FLASH_LED_IRES12P5_MAX_CURR_MA 1600 define MAX_IRES_LEVELS 4 define 
#FLASH_BST_PWM_OVRHD_MIN_UV 300000 define FLASH_BST_PWM_OVRHD_MAX_UV 600000
/* notifier call chain for flash-led irqs */ static 
ATOMIC_NOTIFIER_HEAD(irq_notifier_list); enum flash_charger_mitigation {
...
//start struct flashlight_node_data {
	struct platform_device	*pdev;
...
static int qpnp_flash_led_masked_read(struct qpnp_flash_led *led, u16 addr, u8 
mask,
								u8 *val) {
	int rc;
...
static int qpnp_flash_led_masked_write(struct qpnp_flash_led *led, u16 addr, u8 
mask,
								u8 val) {
	int rc;
...
static enum led_brightness qpnp_flash_led_brightness_get(struct led_classdev 
*led_cdev) {
...
#define VOLTAGE_HDRM_DEFAULT_MV 350 define BHARGER_VOLTAGE_HDRM_DEFAULT_MV 400 
...
#define UCONV 1000000LL define MCONV 1000LL define FLASH_VDIP_MARGIN 50000 define 
#BOB_EFFICIENCY 900LL define VIN_FLASH_MIN_UV	3300000LL
...
#define CHGBST_EFFICIENCY 800LL define CHGBST_FLASH_VDIP_MARGIN 10000 define 
#VIN_FLASH_UV 5000000 define BHARGER_FLASH_LED_MAX_TOTAL_CURRENT_MA 1500 define 
#BHARGER_FLASH_LED_WITH_OTG_MAX_TOTAL_CURRENT_MA	1100
static int qpnp_flash_led_calc_bharger_max_current(struct qpnp_flash_led *led,
	...
	*max_current = min(flash_led_max_total_curr_ma,
			(int)(div64_s64(avail_flash_ua, MCONV)));
	pr_debug("avail_iflash=%lld, ocv=%d, ibat=%d, rbatt=%d, trigger_lmh=%d 
max_current=%lld usb_present=%d otg_enable=%d\n",
		avail_flash_ua, ocv_uv, ibat_now, rbatt_uohm, led->trigger_lmh,
...
static int qpnp_flash_led_symmetry_config(struct flash_switch_data *snode) {
	...
	if (num_leds > 0 && total_curr_ma > 0) {
		...
		pr_err("Incorrect configuration, num_leds: %d total_curr_ma: 
%d\n",
...
static int qpnp_flash_poll_vreg_ok(struct qpnp_flash_led *led) {
	...
	for (i = 0; i < 60; i++) {
		...
		if (rc < 0) {
			pr_err("main psy doesn't support reading prop %d rc = 
%d\n",
...
static int qpnp_flash_led_switch_set(struct flash_switch_data *snode, bool on) {
	...
	if (snode->symmetry_en) {
		...
		if (rc < 0) {
			pr_err("Failed to configure current symmetrically, 
rc=%d\n",
				rc);
			return rc;
		}
	...
	if (options & ENABLE_REGULATOR) {
		if (pmic_subtype == PMI632_SUBTYPE) {
			...
			rc = power_supply_set_property(led->main_psy,
					 ...
					 &ret);
			if (rc < 0) {
				pr_err("Failed to set FLASH_ACTIVE on charger 
rc=%d\n",
									rc);
				return rc;
			}
...
//start
	if(!strcmp("led:switch_0",led_cdev->name) && !value)
		if(NULL != led->flashlight_node)
			led->flashlight_node->cdev.brightness = value; //end
...
//start static void qpnp_flashlight_led_brightness_set(struct led_classdev 
*led_cdev,
						enum led_brightness value) {
	...
	if (!strcmp("flashlight", led_cdev->name)) {
		flashlight_data = container_of(led_cdev, struct 
flashlight_node_data, cdev);
	...
	if (flashlight_data) {
		for(i = 0; i < flashlight_data->num_switch; ++i)
			for(j = 0; j < led->num_snodes; ++j) {
				pr_debug(" switch name[%d] = %s, snode name[%d] = 
%s\n",i,
					flashlight_data->switch_name[i],j, 
led->snode[j].cdev.name);
				if(!strcmp(flashlight_data->switch_name[i], 
led->snode[j].cdev.name)) {
					rc = 
qpnp_flash_led_switch_set(&led->snode[j], false);
				if (rc < 0)
					pr_err("Failed to set flash LED switch 
rc=%d\n", rc);
					break;
				}
			}
		for(i = 0; i < flashlight_data->num_torch; ++i)
			for(j = 0; j < led->num_fnodes; ++j) {
				pr_debug(" torch name[%d] = %s, fnode name[%d] = 
%s\n",i,
					flashlight_data->torch_name[i],j, 
led->fnode[j].cdev.name);
				if(!strcmp(flashlight_data->torch_name[i], 
led->fnode[j].cdev.name)) {
					qpnp_flash_led_node_set(&led->fnode[j], 
value);
					break;
				}
			}
...
led->snode[j].cdev.name)) {
					rc = 
qpnp_flash_led_switch_set(&led->snode[j], value > 0);
				if (rc < 0)
					pr_err("Failed to set flash LED switch 
rc=%d\n", rc);
					break;
...
/* sysfs show function for flash_max_current */ static ssize_t 
qpnp_flash_led_max_current_show(struct device *dev,
...
/* sysfs attributes exported by flash_led */ static struct device_attribute 
...
/* irq handler */ static irqreturn_t qpnp_flash_led_irq_handler(int irq, void 
*_led) {
	...
	if (irq_type == LED_FAULT_IRQ) {
		...
		if (led_status1)
			pr_emerg("led short/open fault detected! 
led_status1=%x\n",
				led_status1);
...
static int qpnp_flash_led_parse_each_led_dt(struct qpnp_flash_led *led,
	...
	if (!rc) {
		if (val < min_ma || val > fnode->max_current)
			pr_warn("Invalid operational current specified, capping 
it\n");
		if (val < min_ma)
			val = min_ma;
	...
	if (fnode->strobe_sel == HW_STROBE) {
		if (of_find_property(node, "qcom,hw-strobe-gpio", NULL)) {
			...
			if (IS_ERR_OR_NULL(fnode->hw_strobe_state_active)) {
				pr_err("No active pin for hardware strobe, 
rc=%ld\n",
			...
			if (IS_ERR_OR_NULL(fnode->hw_strobe_state_suspend)) {
				pr_err("No suspend pin for hardware strobe, 
rc=%ld\n",
...
//start static int qpnp_flashlight_led_parse_and_register(struct qpnp_flash_led 
*led,
			struct flashlight_node_data *flashlight_node, struct 
device_node *node) {
	int rc,i;
	u32 val,count;
...
flashlight_node->num_torch);
	temp_name = kzalloc(sizeof(char *) * count, GFP_KERNEL);
	if(temp_name) {
		for(i = 0; i < count; ++i) {
			flashlight_node->torch_name = temp_name;
			temp_name[i] = kzalloc(sizeof(char) * 
NAME_SIZE,GFP_KERNEL);
			rc = of_property_read_string_index(node,"qcom,torch-name", 
i,&temp_name[i]);
			pr_debug("%s torch_name[%d] = %s\n", __func__, 
i,flashlight_node->torch_name[i]);
		}
	} else
...
NAME_SIZE,GFP_KERNEL);
			rc = 
of_property_read_string_index(node,"qcom,switch-name", i,&temp_name[i]);
			pr_debug("%s switch_name[%d] = %s\n", __func__, 
i,flashlight_node->switch_name[i]);
		}
	} else
...
//end static int qpnp_flash_led_parse_and_register_switch(struct qpnp_flash_led 
*led,
						struct flash_switch_data *snode,
	...
	for_each_available_child_of_node(node, temp) {
		...
		if (!strcmp("switch", temp_string)) {
			rc = qpnp_flash_led_parse_and_register_switch(led,
					&led->snode[j], temp);
			if (rc < 0) {
				pr_err("Unable to parse and register switch node, 
rc=%d\n",
					rc);
...
//start
		if (!strcmp("flashlight", temp_string)) {
			...
			if (rc < 0) {
				pr_err("Unable to parse and register flashlight 
node, rc=%d\n",
					rc);
				goto error_switch_register;
			}
...
//end
	...
	if (led->pdata->all_ramp_up_done_irq >= 0) {
		...
		if (rc < 0) {
			pr_err("Unable to request all_ramp_up_done(%d) 
IRQ(err:%d)\n",
	...
	if (led->pdata->all_ramp_down_done_irq >= 0) {
		...
		if (rc < 0) {
			pr_err("Unable to request all_ramp_down_done(%d) 
IRQ(err:%d)\n",
	...
	i = led->num_snodes; unreg_notifier:
	power_supply_unreg_notifier(&led->nb); error_switch_register:
	while (i > 0)
		led_classdev_unregister(&led->snode[--i].cdev);
	i = led->num_fnodes; error_led_register:
	while (i > 0)
...
late_initcall(qpnp_flash_led_init); static void __exit qpnp_flash_led_exit(void) {
	platform_driver_unregister(&qpnp_flash_led_driver);
}
module_exit(qpnp_flash_led_exit); MODULE_DESCRIPTION("QPNP Flash LED driver v2"); 
